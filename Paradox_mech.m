...
for i = 1:length(alpha_vec)
    ...
    xlim([-1.5, 2]);
    ylim([-0.5, 2]);
    title(sprintf('α = %.1f°, Branch %d', rad2deg(alpha), current_branch));
    xlabel('X');
    ylabel('Y');
    ...
    plot(0, CO1, 'ko', 'MarkerSize', 10, 'MarkerFaceColor', 'k'); % Point O1
    ...
    % Ещё стержни
    plot([0, xD], [CO1, yD], 'k-', 'LineWidth', 2); % O1D
    plot([xD, xM(alpha)], [yD, yM(alpha)], 'k-', 'LineWidth', 2); % DM
    
    % Ещё шарниры
    plot(xA(alpha), yA(alpha), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k'); % A
    plot(xB(alpha), yB(alpha), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k'); % B
    plot(xM(alpha), yM(alpha), 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k'); % M
    plot(xD, yD, 'ko', 'MarkerSize', 8, 'MarkerFaceColor', 'k'); % D
...
%просто доп прорисовка траектории
for i = 1:length(alpha_vec)
    alpha = alpha_vec(i);
    current_branch = branch(i);
    hold on;
    ...
    xlabel('X');
    ylabel('Y');
    plot(xM(alpha), yM(alpha), 'ko', 'MarkerSize', 2, 'MarkerFaceColor', 'k'); % M
